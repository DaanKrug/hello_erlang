-module(masternode_handler).

-behaviour(handler_behaviour).

-export([
		 init/2,
		 object_class_name/0,
		 get_base_url/0,
		 get_max_access_times_on_minute/0,
		 get_categories/0,
		 get_table_name/0,
		 authorize_operation/3,
		 validate_to_save/1,
		 validate_to_update/2,
		 validate_to_delete/1,
		 validate_to_restore/1,
		 validate_to_trully_delete/1,
		 save/3,
		 edit/1,
		 load_all/4,
		 load_all_for_public/4,
		 update/4,
		 delete/3,
		 restore/3,
		 trully_delete/3,
		 need_auth/2,
		 skip_validate_ownership/4,
		 authenticate/3,
		 logoff/2,
		 special_op1/3,
		 special_op2/3
		]).

init(Req, Opts) ->
	rest_handler:handle_request(Req, Opts, ?MODULE).

object_class_name() ->
	<<"Master Node">>.

get_base_url() -> 
	<<"/masternodes">>.

get_max_access_times_on_minute() ->
	25.

get_categories() ->
	[<<"admin_master">>].

get_table_name() ->
	<<"masternode">>.

authorize_operation(_BodyPropList, _UrlParams, _MethodToCall) ->
	true.

need_auth(_BodyPropList, _MethodToCall) ->
	true.

skip_validate_ownership(_MethodToCall, _OwnerId, _Token, _UrlParams) ->
	false.

validate_to_save(BodyPropList) -> 
	A1name = masternode_dataextractor:get_a1_name(BodyPropList, ""), 
	A2ip = masternode_dataextractor:get_a2_ip(BodyPropList, ""),
	A3region = masternode_dataextractor:get_a3_region(BodyPropList, ""),
	A5walletid = masternode_dataextractor:get_a5_walletid(BodyPropList, ""),
	A6walletidentifier = masternode_dataextractor:get_a6_walletidentifier(BodyPropList, ""),
	OwnerId = generic_dataextractor:get_owner_id(BodyPropList),
    if
        ((OwnerId < 1) or (A5walletid < 1)) -> [messages_util:system_message(412)];
		((A1name == "") or (A2ip == "") or (A3region == "") or (A6walletidentifier == "")) 
			-> [messages_util:system_message(412)];
        true -> 
			Wallets = wallet_dao:load_object_by_id(A5walletid),
			case (length(Wallets) == 0) of
				true ->
					[messages_util:system_message(100287)];
				_ ->
					AlreadyExists = masternode_dao:already_exists(<<"0">>, A5walletid),
					case AlreadyExists of
						true ->
							[messages_util:system_message(100290)];
						_ ->
							[]
					end
			end
    end.

validate_to_update(BodyPropList, Id) -> 
	Id2 = number_util:to_integer(Id),
	OwnerId = generic_dataextractor:get_owner_id(BodyPropList),
	if
        ((Id2 < 1) or (OwnerId < 1)) -> [messages_util:system_message(412)];
	    true -> 
			MasterNode = lists:nth(1, edit(Id)),
			A5walletid = proplists:get_value(<<"a5_walletid">>, MasterNode, 0),
			AlreadyExists = masternode_dao:already_exists(Id, A5walletid),
			case AlreadyExists of
				true ->
					[messages_util:system_message(100290)];
				_ ->
					[]
			end
    end.

validate_to_delete(Id) -> 
	Id2 = number_util:to_integer(Id),
	if
        (Id2 < 1) -> [messages_util:system_message(412)];
	    true -> []
    end.

validate_to_restore(Id) -> 
	Id2 = number_util:to_integer(Id),
	if
        (Id2 < 1) -> [messages_util:system_message(412)];
	    true -> 
			MasterNode = lists:nth(1, edit(Id)),
			A5walletid = proplists:get_value(<<"a5_walletid">>, MasterNode, 0),
			AlreadyExists = masternode_dao:already_exists(Id, A5walletid),
			case AlreadyExists of
				true ->
					[messages_util:system_message(100290)];
				_ ->
					[]
			end
    end.

validate_to_trully_delete(Id) -> 
	Id2 = number_util:to_integer(Id),
	if
        (Id2 < 1) -> [messages_util:system_message(412)];
	    true -> []
    end.

save(BodyPropList, OwnerId, Token) -> 
	A1name = masternode_dataextractor:get_a1_name(BodyPropList, ""), 
	A2ip = masternode_dataextractor:get_a2_ip(BodyPropList, ""),
	A3region = masternode_dataextractor:get_a3_region(BodyPropList, ""),
	A4master = generic_dataextractor:get_bool(BodyPropList, <<"a4_master">>, false),
	A5walletid = masternode_dataextractor:get_a5_walletid(BodyPropList, ""),
	A6walletidentifier = masternode_dataextractor:get_a6_walletidentifier(BodyPropList, ""),
	Created = masternode_dao:create_object(A1name, A2ip, A3region, A4master, A5walletid, 
										   A6walletidentifier, OwnerId),
	case lists:nth(1, Created) of 
		true ->
			Id = lists:nth(2, Created),
			Object = lists:nth(1, edit(Id)),
			applog_util:successfully_created(OwnerId, Object, object_class_name(), Token),
			[messages_util:system_message(200)];
		_ ->
			[messages_util:system_message(100288)]
	end.

update(BodyPropList, Id, OwnerId, Token) -> 
	MasterNode = lists:nth(1, edit(Id)),
	A1name = masternode_dataextractor:get_a1_name(BodyPropList, 
												  proplists:get_value(<<"a1_name">>, MasterNode, "")), 
	A2ip = masternode_dataextractor:get_a2_ip(BodyPropList, 
											  proplists:get_value(<<"a2_ip">>, MasterNode, "")), 
	A3region = masternode_dataextractor:get_a3_region(BodyPropList, 
													  proplists:get_value(<<"a3_region">>, MasterNode, "")),
	A4master = generic_dataextractor:get_bool(BodyPropList, <<"a4_master">>, 
											  proplists:get_value(<<"a4_master">>, MasterNode, false)),
	Active = generic_dataextractor:get_bool(BodyPropList, <<"active">>, 
											proplists:get_value(<<"active">>, MasterNode, false)),
	Updated = masternode_dao:update_object(Id, A1name, A2ip, A3region, A4master, Active),
	case Updated of 
		true ->
			ObjectNew = lists:nth(1, edit(Id)),
			applog_util:successfully_updated(OwnerId, MasterNode, ObjectNew, object_class_name(), Token),
			[messages_util:system_message(201)];
		_ ->
			[messages_util:system_message(100289)]
	end.

edit(Id) -> 
	masternode_dao:load_object_by_id(Id).

load_all(BodyPropList, AdditionalConditions, Page, Rows) -> 
	ConditionsAndFlagAsDeleted = generic_dataextractor:get_conditions(BodyPropList),
	Conditions = lists:nth(1, ConditionsAndFlagAsDeleted),
	Conditions2 = lists:concat([Conditions, " ", AdditionalConditions]),
	FlagAsDeleted = lists:nth(2, ConditionsAndFlagAsDeleted),
	masternode_dao:load_all_objects(Page, Rows, Conditions2, FlagAsDeleted).

load_all_for_public(BodyPropList, AdditionalConditions, Page, Rows) -> 
	ConditionsAndFlagAsDeleted = generic_dataextractor:get_conditions(BodyPropList),
	Conditions = lists:nth(1, ConditionsAndFlagAsDeleted),
	Conditions2 = lists:concat([
								Conditions, 
								" ", 
								AdditionalConditions,
								" and a4_master = false and active = true "
							   ]),
	FlagAsDeleted = lists:nth(2, ConditionsAndFlagAsDeleted),
	masternode_dao:load_all_objects_for_public(Page, Rows, Conditions2, FlagAsDeleted).

delete(Id, OwnerId, Token) -> 
	case masternode_dao:delete_object(Id) of 
		true ->
			Object = lists:nth(1, edit(Id)),
			applog_util:successfully_deleted(OwnerId, Object, object_class_name(), Token),
			[messages_util:system_message(204)];
		_ ->
			[messages_util:system_message(500)]
	end.

restore(Id, OwnerId, Token) -> 
	case masternode_dao:restore_object(Id) of 
		true ->
			Object = lists:nth(1, edit(Id)),
			applog_util:successfully_restored(OwnerId, Object, object_class_name(), Token),
			[messages_util:system_message(207)];
		_ ->
			[messages_util:system_message(500)]
	end.

trully_delete(Id, OwnerId, Token) -> 
	Object = lists:nth(1, edit(Id)),
	case masternode_dao:trully_delete_object(Id) of 
		true ->
			applog_util:successfully_trully_deleted(OwnerId, Object, object_class_name(), Token),
			[messages_util:system_message(208)];
		_ ->
			[messages_util:system_message(500)]
	end.

authenticate(_BodyPropList, _Host, _Token) ->
	false.

logoff(_BodyPropList, _Token) ->
	false.

special_op1(_BodyPropList, _OwnerId, _Token) ->
	[messages_util:system_message(403)].

special_op2(_BodyPropList, _OwnerId, _Token) ->
	[messages_util:system_message(403)].
