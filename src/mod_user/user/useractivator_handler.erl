...
object_class_name() ->
	<<"Ativa&ccedil;&atilde;o Usu&aacute;rio">>.
...
special_op1(BodyPropList, OwnerId, Token) ->
	[activate(BodyPropList, OwnerId, Token)].
...
activate(BodyPropList, OwnerId, Token) ->
	...
	if
		...
		true -> activate_user(Email, Password, ConfirmationCode, OwnerId)
	end.

activate_user(Email, Password, ConfirmationCode, OwnerId) ->
    Users = user_dao:load_for_activation(Email, ConfirmationCode),
	case (length(Users) == 0) of
		true ->
			messages_util:system_message(100008);
		_ ->
			User = lists:nth(1, Users),
			HashedPassword = proplists:get_value(<<"password">>, User),
			case (hash_util:password_match(HashedPassword, Password)) of
				true ->
					case (user_dao:activate_user(Email)) of
						true ->
							sucessfully_activated(User, OwnerId);
						_ ->
							messages_util:system_message(100009)
					end;
				_ ->
					messages_util:system_message(100008)
			end
...
sucessfully_activated(User, OwnerId) ->
	...
	case applog_dao:create_object(Id, Name, Email, "Ativou Acesso", "Pessoa/Usu&aacute;rio", Encoded, "") of
		true ->
