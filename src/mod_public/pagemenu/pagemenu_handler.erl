...
validate_to_save(BodyPropList) -> 
    ...
    if
        ((OwnerId < 1) or (Position < 0)) -> [messages_util:system_message(412)];
		(Name == "") -> [messages_util:system_message(100031)];
        true -> []
    end.

validate_to_update(BodyPropList, Id) -> 
	PageMenu = lists:nth(1, edit(Id)),
	Name = pagemenu_dataextractor:get_name(BodyPropList, 
										   proplists:get_value(<<"name">>, PageMenu, "")),
    Position = generic_dataextractor:get_position(BodyPropList, 
												  proplists:get_value(<<"position">>, PageMenu, 0)),
	OwnerId = generic_dataextractor:get_owner_id(BodyPropList),
	if
        ((Id < 1) or (OwnerId < 1) or (Position < 0)) -> [messages_util:system_message(412)];
		(Name == "") -> [messages_util:system_message(100031)];
	    true -> []
    end.
...
validate_to_delete(Id) -> 
	if
	    ...
	    true -> 
			case (pagemenuitem_dao:page_menu_is_in_pagemenuitem(Id)) of
				true ->
					[messages_util:system_message(100112)];
				_ ->
					[]
			end
    end.
...
save(BodyPropList, OwnerId, Token) -> 
	...
	OwnerId = generic_dataextractor:get_owner_id(BodyPropList),
	Created = pagemenu_dao:create_object(Name, Position, Active, OnlyAuth, OwnerId),
	case lists:nth(1, Created) of 
		...
		_ ->
			[messages_util:system_message(100032)]
	end.
...
update(BodyPropList, Id, OwnerId, Token) -> 
	...
	Active = generic_dataextractor:get_bool(BodyPropList, <<"active">>, 
                                            proplists:get_value(<<"active">>, PageMenu, false)),
    OnlyAuth = generic_dataextractor:get_bool(BodyPropList, <<"onlyAuth">>, 
											  proplists:get_value(<<"onlyAuth">>, PageMenu, false)),
	Updated = pagemenu_dao:update_object(Id, Name, Position, Active, OnlyAuth),
	case Updated of 
		true ->
			ObjectNew = lists:nth(1, edit(Id)),
			applog_util:successfully_updated(OwnerId, PageMenu, ObjectNew, object_class_name(), Token),
			[messages_util:system_message(201)];
		_ ->
			[messages_util:system_message(100033)]
	end.

edit(Id) -> 
	pagemenu_dao:load_object_by_id(Id).
...
load_all(BodyPropList, AdditionalConditions, Page, Rows) -> 
	...
	pagemenu_dao:load_all_objects(Page, Rows, Conditions2, FlagAsDeleted).
...
load_all_for_public(BodyPropList, AdditionalConditions, Page, Rows) -> 
	...
	Conditions2 = lists:concat([Conditions, " ", AdditionalConditions]),
	FlagAsDeleted = lists:nth(2, ConditionsAndFlagAsDeleted),
	pagemenu_dao:load_all_objects_for_public(Page, Rows, Conditions2, FlagAsDeleted).

delete(Id, OwnerId, Token) -> 
	case pagemenu_dao:delete_object(Id) of 
		true ->
			Object = lists:nth(1, edit(Id)),
...
restore(Id, OwnerId, Token) -> 
	case pagemenu_dao:restore_object(Id) of 
		true ->
			Object = lists:nth(1, edit(Id)),
...
trully_delete(Id, OwnerId, Token) -> 
	Object = lists:nth(1, edit(Id)),
	case pagemenu_dao:trully_delete_object(Id) of 
		true ->
